...
task download_fasta {
  input {
    ...
    String         docker = "quay.io/broadinstitute/viral-ngs:3.0.4-phylo"
  }
...
task table2asn {
  ...
  input {
    ...
    String       docker = "quay.io/broadinstitute/viral-ngs:3.0.4-phylo"  # this could be a simpler docker image, we don't use anything beyond table2asn itself
  }
  Int disk_size = 50
...
task package_special_genbank_ftp_submission {
  ...
  input {
    ...
    String  docker = "quay.io/broadinstitute/viral-ngs:3.0.4-baseimage"
  }
  command <<<
    set -e
