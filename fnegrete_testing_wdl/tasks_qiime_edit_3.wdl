...
task qiime_import_from_bam {
    ...
    output {
        ...
        String samplename-master-sheet = read-string("NEWSAMPLENAME.txt")
    }
...
task merge-paired-ends { 
    meta {
        description: "Join paired-end sequence reads using vseach's merge-pairs function. Perform sequence quality control for Illumina data using the Deblur workflow with a 16S reference as a positive filter."
        }
    input {
...
task gen-feature-table {
    ...
    input {
        ...
        Int    trim-length-var = 300     
        String  docker = "quay.io/qiime2/core:2022.8"
    }
    command <<< 
            set -ex -o pipefail
            CONDA-ENV-NAME=$(conda info --envs -q | awk -F" " '/qiime.*/{ print $1 }')
            conda activate ${CONDA-ENV-NAME}
            ##activated conda environments 
            qiime deblur denoise-16S \
            --i-demultiplexed-seqs "~{joined_end_basename}_demux_joined.qza" \ 
            ~{"--p-trim-length" + trim-length-var} \ 
            --p-sample-stats \
    ...
    output {
        ...
        File feature-table = "~{joined_end_basename}_table.qzv"
        File visualize-stats = "~{joined_end_basename}_deblur_stats.qzv"
...
task train-classifier {
    ...
    input {
        ...
        Int    min_length  =   100
        Int    max-length  =   500
        String  otu-basename    =   basename(otu_ref, '.qza')
    ...
    command <<<
     set -ex -o pipefail
        ...
        --input-path ~{otu_ref} \
        --output-path "~{otu-basename}_seqs.qza"
        ...
        --output-path "~_v{otu-basename}_tax.qza"

        qiime feature-classifier extract-reads\
        --i-sequeunces "~{otu-basename}_seqs.qza"\
        --p-f-primer "~{forward_adapter}"\
        --p-r-primer "~{reverse_adapter}"\
        ~{"--p-min_length" + min_length}\
        ~{"--p-max-length" + max-length}\
        --o-reads "~{otu-basename}_v1_2_ref_seqs.qza"
#might have to be broken down into two tasks
        qiime feature-classifier fit-classifier-naive-bayes\ 
        --i-reference-reads "~{otu-basename}_v1_2_ref_seqs.qza"\ 
        --i-reference-taxonomy "~_v{otu-basename}_tax.qza"\ 
        --o-classifier "~{otu-basename}_v1_2_classifier.qza"
    >>>
    output {
        File    trained_classifier = "~{otu-basename}_v1_2_classifier.qza"
    }
    runtime {
        docker: "~{docker}"
        memory: "10 GB"
...
task tax-analysis {
    ...
    >>>
    output {
        File rep-seq-list = "~{basename}_rep_seqs.qzv" 
        File tax-classification-graph = "~{basename}_bar_plots.qzv"
    }
    runtime {
        docker: "~{docker}"
        memory: "10 GB"
