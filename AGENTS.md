...
- **No agent attribution in commit messages.** Do not add `Co-Authored-By` or similar agent/bot attribution lines to commit messages. Agent attribution is appropriate in PR descriptions, issue comments, and other discussion contexts, but not in the permanent commit log.
- Prefer specific `git add <file>` over `git add -A` or `git add .` to avoid accidentally staging unrelated files.
